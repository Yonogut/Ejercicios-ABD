...
plot(theta, densidad_prior, type = "l", col = "blue", lwd = 2,
     ...
     xlab = "Tasa de aceptación (theta)", ylab = "Densidad")
...
cat("Distribución posterior: Normal(", round(mu, 2), ",", round(sigma, 2), ")\n")
```
...
sigma <- (varianza_previa * varianza_v)/(varianza_previa*n + varianza_v) 
mu <- (varianza_v * media_previa + varianza_previa*n*media_muestral)/(varianza_v + varianza_previa*n)
x <- seq(media_previa - 4 * sqrt(varianza_previa), media_previa + 4 * sqrt(varianza_previa), length.out = 1000)
densidad_previa <- dnorm(x, mean = media_previa, sd = sqrt(varianza_previa))
densidad_posterior <- dnorm(x, mean = mu, sd = sqrt(sigma))
...
```
La solución analítica y gráfica son consistentes, ya que el modo (pico en la gráfica) de la distribución posterior coincide con la media posterior calculada analíticamente (228.4), y la varianza posterior es idéntica en ambos métodos. Esto demuestra que ambos enfoques reflejan adecuadamente la misma información.
:::
