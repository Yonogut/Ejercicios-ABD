...
```{r valor maximo de la funcion de densidad normal}
dnorm(0) # valor maximo para la distibucion normal
```
```{r alor maximo de la funcion de densidad uniforme}
...
```{r valor orientativo norm}
dnorm(0,0,0.1)
```
```{r valor orientativo uniform}
...
x <- seq(-4, 4, 1e-3)
densidad <- dnorm(x)
cdf_aproximada <- cumsum(densidad) * 1e-3
...
lines(theta, densidad_posterior, col = "red", lwd = 2)
par(xpd = TRUE)
legend("bottomleft",inset = c(-0.1,- 0.5), cex = 0.7,legend = c("Distribución Previa (Beta(18,6))", 
...
varianza_v <- varianza_muestral
sigma <- (varianza_previa * varianza_v)/(varianza_previa*n + varianza_v) 
mu <- (varianza_v * media_previa + varianza_previa*n*media_muestral)/(varianza_v + varianza_previa*n) 
...
set.seed(123)
n_samples <- 100000
theta_samples <- rnorm(n_samples, mean = media_previa, sd = sqrt(varianza_previa / n_muestra)) 

weights <- dnorm(media_muestral, mean = theta_samples, sd = sqrt(varianza_muestral / n_muestra))  

weights <- weights / sum(weights)

expected_value_mc <- sum(theta_samples * weights)
expected_value_squared_mc <- sum((theta_samples^2) * weights)
variance_posterior_mc <- expected_value_squared_mc - expected_value_mc^2

sd_posterior_mc <- sqrt(variance_posterior_mc)

cat("Estimador esperado :", round(expected_value_mc, 2), "\n")
cat("Desviación estándar posterior:",round(sd_posterior_mc, 2))
...
densidad_posterior <- dnorm(x, mean = expected_value_mc, sd =sd_posterior_mc)
plot(x, densidad_previa, type = "l", col = "blue", lwd = 2, 
     ylab = "Densidad", xlab = "Media",ylim = c(0,0.12), main = "Distribuciones Previa y Posterior")
lines(x, densidad_posterior, col = "red", lwd = 2)
par(xpd = TRUE)
legend("bottomleft",inset = c(-0.15,- 0.45), cex = 0.7, legend = c("Distribución Previa (Normal(247, 1156))", 
                              "Distribución Posterior (Normal(228.4, 36.12))"),
       col = c("blue", "red"), lwd = 2, bty = "n")
```
La diferencia entre ambos métodos se debe a cómo calculan la distribución posterior. El método analítico usa la actualización bayesiana exacta, mientras que el método de integracion numerica estima la media y varianza a partir de muestras ponderadas. La discrepancia en los valores (224.4 vs. 239.75 para la media y 6.01 vs. 3.75 para la desviación estándar) sugiere que el metodo de integracion numerica puede estar subestimando la varianza, posiblemente debido a una mala normalización de pesos o una baja superposición entre la previa y la verosimilitud.
:::
